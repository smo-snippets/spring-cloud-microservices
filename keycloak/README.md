...
3. Add a new realm named `smotastic`.
4. Import `realm-export.json`.
5. On the `test-client` client Credentials tab, `Regenerate Secret` and store value in gateway `application.properties` (only if not public-client).
6. Add a new user with username `user`.
7. Under credentials set user password to `password` with `Temporary: OFF`.

## What you'll get

* a realm named `smotastic`.
* a client named `test-client` with:
